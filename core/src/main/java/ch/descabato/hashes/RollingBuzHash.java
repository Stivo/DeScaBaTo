...
public class RollingBuzHash {
    ...
    public void update(byte[] bs, int offset, int len) {
        for (int i = offset; i < len + offset; i++) {
            byte b = bs[i];
            hash = (hash << 1) ^ (hash >>> 63) ^ mix_master[(b ^ (b >>> 8)) & 0xff];
            int zeroBits = countZeroBits(hash);
            counts[zeroBits] += 1;
            if (zeroBits > 21) {
                System.out.println(Long.toBinaryString(hash));
                System.out.println("Found " + zeroBits + " zero bits at " + (bytesSeen + i));
            }
        }
        bytesSeen += len;
    }
    ...
    private int countZeroBits(long hash) {
        int count = 0;
        if ((hash & 0xFFFFF) == 0) {
            count += 20;
            hash = hash >> 20;
    ...
    public void printStatistics() {
        for (int i = 0; i < 32; i++) {
            ...
            System.out.println("Count for " + i+" is "+countHere);
        }
    }
